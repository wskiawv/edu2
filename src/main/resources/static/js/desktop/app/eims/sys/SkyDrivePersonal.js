 ...
 * @class Desktop.
 */
Ext.define('desktop.app.eims.sys.SkyDrivePersonal', {
	...
	id : 'SkyDrivePersonal',
	windowId : 'SkyDrivePersonal',
	title:'公共网盘',
	tipWidth : 160,
	tipHeight : 96,
	rootDir:Boolean(localFlag)?localPath+"/"+userId:hdfsUrl+"/"+userId,
	dir:"",
	init : function() {
		var me=this;
		this.launcher = {
	...
	searchPanel:function(){
		var searchpanel=Ext.create('Ext.form.Panel',{
			...
			buttons:[{
					...
					iconCls:'search',
					handler : function() {						
						var store = Ext.data.StoreManager.lookup('SkyDrivePersonalStore');
						store.load();						
	...
	createGridStore:function(){
		var store = Ext.create('Ext.data.JsonStore', {
					storeId : 'SkyDrivePersonalStore',
					//autoLoad : true,
					pageSize: 20,
					model:'desktop.app.eims.sys.model.SkyDrivePersonal',
					//scope:me,
					listeners:{
						//scope:me,
						beforeload:function(s,operation,eOpts){
							//var form = me.searchPanel().getForm(),
							var form=Ext.getCmp('psp').getForm(),
							_params = form.getValues() || {},										
							bsParams = {start: 0, limit: 20},
							params = {};
							
							//grid = me.createGridPanel(),
							var store1 = Ext.data.StoreManager.lookup('SkyDrivePersonalStore');
							...
							Ext.apply(s.proxy.extraParams,Ext.getCmp('psp').getForm().getValues());
	...
	createGridPanel : function() {
		...
		var Grid = Ext.create('Ext.grid.Panel', {
					...
					dockedItems : [{
								xtype : 'pagingtoolbar',
								store : Ext.data.StoreManager.lookup('SkyDrivePersonalStore'), // same store
								// GridPanel is
								// using
								dock : 'bottom',
								displayInfo : true,
                	 ...
                	 xtype:'form',                	
         			 ...
         			 buttons:[{
         					...
         					handler : function() {						
         						...
         						if (form.isValid()) {
    								form.submit({										
    									url : 'sys/SkyDrivePersonals/mkdir',
    									waitMsg : '创建目录中',
    									success : function(fp, o) {			
    										try {														
    											Ext.example.msg('温馨提醒',o.result.msg);
    											Ext.data.StoreManager.lookup('SkyDrivePersonalStore').load();
    											win.close();
    										} catch (e) {
    											if (o.status !== "500") {													
    												Ext.example.msg('温馨提醒',o.result.msg);
	...
	remove:function(records){
		...
		Ext.Msg.confirm("温馨提醒","确定删除所选择的数据？",function(btn, text){
			if(btn == 'yes'){
				Ext.Ajax.request({
					url:appPath+"/sys/SkyDrivePersonals/remove",
					params:{data:data},
	...
	onLineShow:function(records,store){
		...
		if(i==0)
 		...
 		{
 			var  me=this,desktop = me.app.getDesktop();
			var swfwindowId="pshowswf"+records[0].get("size");
	        var  win = desktop.getWindow(swfwindowId); 	               	
	       	if(!win){
				win=desktop.createWindow({
					id: swfwindowId,
			...
			if(filextype=="jpg"||filextype=="png"||filextype=="gif"||filextype=="svg"){
				...
				var iframe=Ext.create('Ext.ux.IFrame',{
	        		...
	        		//height:470,
	    	 	 	//src:"show"	        	
	    	  	 	src:"/sys/SkyDrivePersonals/online?id="+records[0].get("id")+"&FilePath="+encodeURI(filepath)
	       		});
				win.add(iframe);			
			}			
			win.show();		 			
	...
	download :function(records){
		var i=records.length;			         	
 		...
 		window.open(appPath+"/sys/SkyDrivePersonals/download?fileName="+records[0].get("FilePath"));
 		/*if(loadOrShow){
	...
	filesUpLoad:function(){
        ...
        var uploadButton=Ext.create('Ext.ux.upload.Button', {
    		...
    		uploader: 
    		{
    			url: appPath+'/sys/SkyDrivePersonals/upload',
    			uploadpath: 'upload',
    			autoStart: false,
                ...
                items:[{
					...
					buttons:[{
						...
						handler : function() {
							...
							hidden.setValue(me.dir);
							//var grid2=me.createGridPanel();
							//var store=grid2.getStore();
							if (form.isValid()) {
								form.submit({										
									url : 'sys/SkyDrivePersonals/upload',
									waitMsg : '正在上传中...',
									success : function(fp, o) {			
										try {														
											Ext.example.msg('温馨提醒',o.result.msg);
 ...
 * 数据模型
 */
Ext.define('desktop.app.eims.sys.model.SkyDrivePersonal',{
